...
<table class="table table-striped table-bordered <?= $detect->isMobile()?"":"table-responsive" ?>">
      ...
      <tbody>
        ...
        <tr id="{{$it->NRO}}"> 
          ...
          <td> 
           <!--MANDAR A IMPRIMIR -->
           <a  href="<?= url("filtro")?>" data-toggle="modal" data-target="#show_opc_rep" onclick="mostrar_informe(event)" style="color:black;" > <i class="mr-2 ml-2 fa fa-print {{$icons_size}}" aria-hidden="true"></i></a> 
          </td>
          <td>{{$it->NRO}}</td>
          <td>{{$it->NOMBRE }}</td>
           </tr>
