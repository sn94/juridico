    ...
    <table id="demandatable" class="table table-bordered table-striped">
      ...
      <?php foreach( $lista as $item): ?>
          <tr> <td> <a href="<?=url("ficha-demanda/".$item->COD_EMP)?>"><?= $item->COD_EMP?></a> </td>  <td><?= $item->CI?></td> <td><?= $item->TITULAR?></td><td><?= $item->DEMANDANTE?></td><td><?= $item->CTA_BANCO?></td><td><?= $item->BANCO?></td><td><?= $item->GARANTE?></td><td><?= $item->CI_GARANTE?></td></tr>
      <?php  endforeach; ?>
      </tbody>
      </table>
      <script>
