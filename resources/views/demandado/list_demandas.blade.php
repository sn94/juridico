...
<div id="tabla-dinamica">
    <table id="demandadostable" class="table table-bordered table-striped">
      <thead class="thead-dark">
          <tr> <th></th> <th></th><th></th><th>DEMANDANTE</th>  <th>ORIGEN</th> <th>PRESENTADO</th> <th>FEC_EMBARGO</th><th>COD_EMP</th>  </tr>
      </thead>
      <tbody>
      <?php foreach( $lista as $item): ?>
          <tr id="{{$item->IDNRO}}"> 
           ...
           <td >  <a onclick="procesar_borrar(event)" href="<?= url("demandas-borrar/".$item->IDNRO)?>"><i class="fa fa-trash" aria-hidden="true"></i></a> </td> 
            <td >  <?= $item->DEMANDANTE?> </td><td > <?= $item->O_DEMANDA?></a> </td> <td>{{$item->PRESENTADO}}</td><td>{{$item->EMB_FECHA}}</td> <td><?= $item->COD_EMP?></td>     </tr>
      <?php  endforeach; ?>
      </tbody>
      </table>
  </div>
